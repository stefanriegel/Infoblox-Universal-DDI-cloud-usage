...
from .config import GCPConfig, get_gcp_credential, validate_gcp_config
...
class GCPDiscovery(BaseDiscovery):
                ...
                try:
                    ...
                    for instance in page_result:
                        ...
                        formatted_resource = self._format_resource(
                            details, 'compute-instance', region, instance_name, 
                            requires_token, 'active', labels
                        )
        ...
        try:
            ...
            if region == self.config.regions[0]:  # Only discover once
                ...
                for network in page_result:
                    ...
                    # Format resource
                    formatted_resource = self._format_resource(
                        details, 'vpc-network', 'global', network_name, 
                        requires_token, 'active', labels
                    )
    ...
    def _discover_subnets(self, region: str) -> List[Dict]:
        ...
        try:
            ...
            for subnet in page_result:
                ...
                # Format resource
                formatted_resource = self._format_resource(
                    details, 'subnet', region, subnet_name, 
                    requires_token, 'active', labels
                )
    ...
    def _discover_cloud_dns_zones_and_records(self) -> List[Dict]:
        ...
        try:
            ...
            for zone in self.dns_client.list_zones():
                ...
                # Format resource
                formatted_resource = self._format_resource(
                    details, 'dns-zone', 'global', dns_name, 
                    requires_token, 'active', {}
                )
    ...
    def _discover_dns_records(self, zone) -> List[Dict]:
        ...
        try:
            for record in zone.list_resource_record_sets():
                ...
                # Format resource
                formatted_resource = self._format_resource(
                    details, 'dns-record', 'global', record_name, 
                    requires_token, 'active', {}
                )
    ...
    def _is_managed_service(self, labels: Dict[str, str]) -> bool:
        ...
        managed_indicators = [
            'goog-managed-by', 'managed-by', 'google-managed',
            'gke-managed', 'cloud-run', 'cloud-functions'
        ]
    ...
    def get_management_token_free_assets(self) -> List[Dict]:
        """Get resources that don't require Management Tokens."""
        if self._discovered_resources is None:
            self.discover_native_objects()
        
        return [resource for resource in self._discovered_resources 
                if not resource.get('requires_management_token', True)]
    
    def calculate_management_token_requirements(self) -> Dict:
        """Calculate Management Token requirements based on discovered resources."""
        if self._discovered_resources is None:
            self.discover_native_objects()
        
        # Use the shared token calculator
        from shared.token_calculator import UnifiedTokenCalculator
        
        calculator = UnifiedTokenCalculator('gcp')
        calculation = calculator.calculate_management_tokens(self._discovered_resources)
    ...
    def save_discovery_results(self, output_dir: Optional[str] = None) -> Dict[str, str]:
        ...
        # Calculate and save token requirements
        ...
        saved_files = {**native_objects_file, **token_files}
