...
class AzureActivityAnalyzer:
    """Analyzes Azure Activity Logs to understand resource growth patterns."""
    
    def __init__(self, subscription_id: str, regions: Optional[List[str]] = None):
        self.subscription_id = subscription_id
        self.regions = regions or ['eastus']
    ...
    def get_resource_events(self, days_back: int = 90) -> Dict[str, List[Dict]]:
        """
        Fetch Azure Activity Log events for resource creation/deletion over the specified period.
        ...
        """
        logger.info(f"Fetching Azure Activity Log events for the last {days_back} days...")
        ...
        event_names = [
            ...
            'Microsoft.Network/virtualNetworks/delete',  # VNet deletion
            'Microsoft.Network/virtualNetworks/subnets/write',  # Subnet creation
            'Microsoft.Network/virtualNetworks/subnets/delete', # Subnet deletion
            'Microsoft.Network/loadBalancers/write',     # Load balancer creation
            'Microsoft.Network/loadBalancers/delete'     # Load balancer deletion
        ]
        ...
        for event_name in event_names:
            try:
                ...
                filter_str = f"eventTimestamp ge '{start_time.isoformat()}' and eventTimestamp le '{end_time.isoformat()}' and eventName eq '{event_name}'"
                
                events = self.monitor_client.activity_logs.list(filter=filter_str)
                ...
                for event in events:
                    ...
                    # Categorize events
                    if 'virtualMachines' in event_name:
                        resource_events['virtual_machines'].append(event_data)
                    elif 'virtualNetworks' in event_name and 'subnets' not in event_name:
                        resource_events['virtual_networks'].append(event_data)
                    elif 'subnets' in event_name:
                        resource_events['subnets'].append(event_data)
                    elif 'loadBalancers' in event_name:
    ...
    def create_growth_dataset(self, resource_events: Dict[str, List[Dict]], days_back: int = 90) -> pd.DataFrame:
        """
        Create a time series dataset from Azure Activity Log events.
        
        Args:
            resource_events: Events from Activity Logs
            days_back: Number of days to analyze
            
        Returns:
            DataFrame with daily resource counts
        """
        logger.info("Creating growth dataset from Azure Activity Log events...")
        ...
        for resource_type, events in resource_events.items():
            ...
            for date in date_range:
                ...
                for event in date_events:
                    if 'write' in event['event_name']:
                        running_count += 1
                    elif 'delete' in event['event_name']:
        ...
        df['total_native_objects'] = (
            df['virtual_machines_count'] + 
            df['virtual_networks_count'] + 
            df['subnets_count'] + 
        ...
        df['management_tokens'] = (
            df['virtual_machines_count'] * 1 +      # 1 token per VM
            df['virtual_networks_count'] * 1 +      # 1 token per VNet
    ...
    def predict_future_growth(self, analysis: Dict, months_ahead: int = 36) -> pd.DataFrame:
        ...
        monthly_predictions = predictions_df.set_index('date').resample('ME').mean()
    ...
    def run_complete_analysis(self, days_back: int = 90, months_ahead: int = 36, 
        ...
        """
        Run complete historical analysis using Azure Activity Logs.
        ...
        """
        logger.info("Starting complete Azure Activity Log-based historical analysis...")
        ...
        logger.info("Complete Azure Activity Log analysis finished")
