...
# Fix Windows console and subprocess encoding to UTF-8
...
def _check_aws_auth() -> int:
    ...
    try:
        proc = subprocess.run(["aws", "--version"], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30)
...
def _check_azure_auth() -> int:
    ...
    try:
        ...
        proc = subprocess.run(az_cmd + ["version"], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30)
...
def _check_gcp_auth() -> int:
    ...
    try:
        proc = subprocess.run(["gcloud", "--version"], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30)
        if proc.returncode == 0:
...
def _configure_logging():
    ...
    for name in ("boto3", "botocore", "urllib3", "azure", "azure.core",
                 "azure.mgmt", "google", "google.auth", "google.cloud"):
