...
def run_licensing_export(
    ...
    """Run Universal DDI licensing calculations and export all artefacts.

    This is the shared logic previously duplicated across
    aws_discovery/discover.py, azure_discovery/discover.py, and
    ...
    calculator.calculate_from_discovery_results(native_objects, provider=provider)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs(output_dir, exist_ok=True)
    exported: Dict[str, str] = {}

    # CSV for Sales Engineers
    csv_file = os.path.join(output_dir, f"{provider}_universal_ddi_licensing_{timestamp}.csv")
    calculator.export_csv(csv_file, provider=provider)
    print(f"Licensing CSV exported: {csv_file}")
    exported["licensing_csv"] = csv_file

    # Text summary
    txt_file = os.path.join(output_dir, f"{provider}_universal_ddi_licensing_{timestamp}.txt")
    calculator.export_text_summary(txt_file, provider=provider)
    print(f"Licensing summary exported: {txt_file}")
    exported["licensing_txt"] = txt_file

    # Estimator CSV
    estimator_csv = os.path.join(output_dir, f"{provider}_universal_ddi_estimator_{timestamp}.csv")
    calculator.export_estimator_csv(estimator_csv)
    print(f"Estimator CSV exported: {estimator_csv}")
    exported["estimator_csv"] = estimator_csv

    # Auditable proof manifest
    proof_file = os.path.join(output_dir, f"{provider}_universal_ddi_proof_{timestamp}.json")
    ...
    exported["proof_manifest"] = proof_file
