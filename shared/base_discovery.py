...
class BaseDiscovery(ABC):
    ...
    def _is_managed_service(self, tags: Dict[str, str]) -> bool:
        """Check if a resource is a managed service using exact tag matches."""
        if not tags:
            return False

        from .constants import MANAGED_SERVICE_INDICATOR_KEYS, MANAGED_SERVICE_INDICATOR_VALUES

        provider = self.config.provider.lower()
        key_indicators = MANAGED_SERVICE_INDICATOR_KEYS.get(provider, set())
        value_indicators = MANAGED_SERVICE_INDICATOR_VALUES.get(provider, set())

        for key, value in tags.items():
            if key.lower() in key_indicators:
                return True
            if isinstance(value, str) and value.lower() in value_indicators:
                return True
