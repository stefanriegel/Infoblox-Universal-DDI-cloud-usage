...
# Keys: exact tag key matches (lowercased).  Values: exact tag value matches (lowercased).
# A resource is considered "managed" if ANY tag key OR tag value appears in the
# corresponding set.  Use exact matching only — substring matching produces false positives.
MANAGED_SERVICE_INDICATOR_KEYS = {
    "aws": {
        "aws:cloudformation:stack-name",
        "aws:eks:cluster-name",
        "aws:autoscaling:groupname",
        "elasticmapreduce:job-flow-id",
        "managed-by",
        "aws-managed",
    },
    "azure": {
...
MANAGED_SERVICE_INDICATOR_VALUES = {
    "aws": {
        "aws-managed",
        "ecs-service",
        "eks",
