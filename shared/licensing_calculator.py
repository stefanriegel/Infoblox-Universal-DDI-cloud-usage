...
class UniversalDDILicensingCalculator:
    ...
    def _count_ddi_objects(self, resources: List[Dict]) -> int:
        ...
        Uses DDI_RESOURCE_TYPES from constants.py as single source of truth.
        """
        # Build combined set from all providers via constants.py
        ddi_resource_types: set[str] = set()
    ...
    def _count_managed_assets(self, resources: List[Dict]) -> int:
        ...
        Uses ASSET_RESOURCE_TYPES from constants.py as single source of truth.
        """
        # Build combined set from all providers via constants.py
        asset_resource_types: set[str] = set()
    ...
    def _is_ddi_object(self, resource_type: str) -> bool:
        """Check if resource type is a DDI object (uses constants.py)."""
        ddi_types: set[str] = set()
        for types in DDI_RESOURCE_TYPES.values():
            ddi_types.update(types)
        return resource_type in ddi_types

    def _is_managed_asset(self, resource_type: str) -> bool:
        """Check if resource type is a managed asset (uses constants.py)."""
